...
class AIInterviewSession(models.Model):
    ...
    interview = models.OneToOneField(
        Interview, on_delete=models.CASCADE, related_name="ai_session"
    )
    ...
    ai_configuration = models.JSONField(
        default=dict, help_text="AI model configuration parameters"
    )
    ...
    def complete_session(self):
        ...
        if self.session_started_at:
            self.session_duration = int(
                (self.session_ended_at - self.session_started_at).total_seconds()
            )
        self.save()
...
class AIInterviewQuestion(models.Model):
    ...
    question_context = models.JSONField(
        default=dict, help_text="Additional context for the question"
    )
    audio_url = models.URLField(
        max_length=500,
        null=True,
        blank=True,
        help_text="URL to the audio file for this question",
    )
    ...
    score = models.FloatField(
        null=True, blank=True, help_text="Score for this question (0-100)"
    )
    ...
    def mark_answered(self, ai_response, response_time=None):
        ...
        if response_time:
            ...
            self.response_time = (
                self.response_received_at - self.question_asked_at
            ).total_seconds()
        self.save()
...
class AIInterviewResponse(models.Model):
    ...
    question = models.ForeignKey(
        AIInterviewQuestion, on_delete=models.CASCADE, related_name="responses"
    )
    session = models.ForeignKey(
        AIInterviewSession, on_delete=models.CASCADE, related_name="responses"
    )

    # Response Content
    response_text = models.TextField()
    response_type = models.CharField(
        max_length=20, default="text", help_text="text, audio, video, code"
    )
    response_data = models.JSONField(
        default=dict,
        help_text="Additional response data (audio/video URLs, code files, etc.)",
    )

    # Timing
    response_submitted_at = models.DateTimeField(default=timezone.now)
    response_duration = models.PositiveIntegerField(
        default=0, help_text="Time taken to respond in seconds"
    )

    # AI Evaluation
    ai_evaluation = models.JSONField(
        default=dict, help_text="AI evaluation of the response"
    )
    ai_score = models.FloatField(
        null=True, blank=True, help_text="AI score for this response (0-100)"
    )
    ai_feedback = models.TextField(blank=True, help_text="AI feedback on the response")

    # Quality Metrics
    response_length = models.PositiveIntegerField(
        default=0, help_text="Length of response in characters"
    )
    confidence_score = models.FloatField(
        null=True, blank=True, help_text="AI confidence in evaluation"
    )
    ...
    def __str__(self):
        return (
            f"Response to Q{self.question.question_index}: {self.response_text[:50]}..."
        )
...
class AIInterviewResult(models.Model):
    ...
    session = models.OneToOneField(
        AIInterviewSession, on_delete=models.CASCADE, related_name="result"
    )
    interview = models.OneToOneField(
        Interview, on_delete=models.CASCADE, related_name="ai_result"
    )

    # Overall Scores
    total_score = models.FloatField(
        default=0.0, help_text="Overall interview score (0-10)"
    )
    technical_score = models.FloatField(
        default=0.0, help_text="Technical questions score (0-10)"
    )
    behavioral_score = models.FloatField(
        default=0.0, help_text="Behavioral questions score (0-10)"
    )
    coding_score = models.FloatField(
        default=0.0, help_text="Coding questions score (0-10)"
    )
    communication_score = models.FloatField(
        default=0.0, help_text="Communication skills score (0-10)"
    )
    problem_solving_score = models.FloatField(
        default=0.0, help_text="Problem solving skills score (0-10)"
    )
    ...
    completion_time = models.PositiveIntegerField(
        default=0, help_text="Total completion time in seconds"
    )

    # AI Analysis
    ai_summary = models.TextField(
        blank=True, help_text="AI-generated interview summary"
    )
    ai_recommendations = models.TextField(blank=True, help_text="AI recommendations")
    strengths = models.JSONField(
        default=list, help_text="Candidate strengths identified"
    )
    weaknesses = models.JSONField(default=list, help_text="Areas for improvement")
    coding_details = models.JSONField(
        default=list,
        blank=True,
        help_text="Detailed coding questions with test results, code, and output logs"
    )
    ...
    overall_rating = models.CharField(
        max_length=20,
        default="pending",
        help_text="excellent, good, average, poor, pending",
    )
    hire_recommendation = models.BooleanField(
        null=True, blank=True, help_text="AI recommendation to hire"
    )
    confidence_level = models.FloatField(
        default=0.0, help_text="AI confidence in evaluation (0-10)"
    )
    ...
    human_reviewer = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="ai_interview_reviews",
    )
    human_rating = models.CharField(
        max_length=20, blank=True, help_text="Human reviewer rating"
    )
    human_feedback = models.TextField(blank=True, help_text="Human reviewer feedback")
    ...
    def calculate_scores(self):
        ...
        for question in questions:
            if question.score is not None:
                ...
                if question.question_type == "technical":
                    ...
                    technical_count += 1
                    # Technical questions also test problem-solving
                    problem_solving_score += question.score
                    problem_solving_count += 1
                elif question.question_type == "behavioral":
                    behavioral_score += question.score
                    behavioral_count += 1
                    # Behavioral questions also test communication
                    communication_score += question.score
                    communication_count += 1
                elif question.question_type == "coding":
                    coding_score += question.score
                    coding_count += 1
                    # Coding questions test both problem-solving and communication (code clarity)
                    problem_solving_score += question.score
                    problem_solving_count += 1
                    communication_score += question.score
                    communication_count += 1
        ...
        if communication_count > 0:
            self.communication_score = communication_score / communication_count
            
        if problem_solving_count > 0:
            self.problem_solving_score = problem_solving_score / problem_solving_count
