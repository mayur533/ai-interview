...
def release_camera_for_session(session_id):
    ...
    # Temporarily disabled for testing
    logger.info(
        f"Camera release functionality temporarily disabled for session {session_id}"
    )
    pass
        ...
        try:
            ...
            if created:
                try:
                    ...
                    logger.info(
                        f"Generated {len(questions)} questions for AI session {ai_session.id}"
                    )
                except Exception as e:
            ...
            return Response(
                {
                    ...
                    "candidate_email": interview.candidate.email,
                    "job_title": (
                        interview.job.job_title if interview.job else "Technical Role"
                    ),
    ...
    def get(self, request, interview_id):
        ...
        try:
            ...
            try:
                ...
                ai_session_id = str(ai_session.id)
                questions_count = AIInterviewQuestion.objects.filter(
                    session=ai_session
                ).count()
            ...
            return Response(
                {
                    ...
                    "is_expired": interview.link_expires_at
                    and timezone.now() > interview.link_expires_at,
        ...
        try:
            ...
            if not created:
                ...
                try:
                    questions = ai_interview_service.generate_questions(ai_session)
                    logger.info(
                        f"Regenerated {len(questions)} questions for AI session {ai_session.id}"
                    )
                except Exception as e:
        ...
        try:
            ...
            if created:
                ...
                try:
                    ...
                    logger.info(
                        f"Generated {len(questions)} questions for session {session.id}"
                    )
                except Exception as e:
                    logger.error(f"Error generating questions: {e}")
                    # Create a default question if AI service fails
            ...
            questions = AIInterviewQuestion.objects.filter(session=session).order_by(
                "question_index"
            )
            ...
            return Response(
                {
                    ...
                    "current_question": (
                        ...
                        if current_question
                        else "Welcome to your Talaro interview! Let's begin."
                    ),
...
class PublicSubmitResponseView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # Validate the interview token
            is_valid, message = validate_interview_token(
                str(session.interview.id), link_token
            )
            if not is_valid:
                return Response({"error": message}, status=status.HTTP_401_UNAUTHORIZED)
            ...
            current_question = (
                AIInterviewQuestion.objects.filter(session=session)
            ...
            # Get next question
            next_question = (
                AIInterviewQuestion.objects.filter(
                    session=session, question_index__gt=current_question.question_index
                )
                .order_by("question_index")
                .first()
            )
            ...
            return Response(
                {
                    ...
                    "next_question": (
                        next_question.question_text if next_question else None
                    ),
                    "feedback": feedback,
            ...
            try:
                ...
                score = (
                    ...
                    if score_match
                        ...
                        100, int((total_responses / max(total_questions, 1)) * 100)
                    )
                )
                evaluation = (
                    evaluation_match.group(1).strip()
                    if evaluation_match
                    else f"Interview completed successfully. You answered {total_responses} out of {total_questions} questions."
                )
                strengths = (
                    strengths_match.group(1).strip()
                    if strengths_match
                    else "Good participation in the interview."
                )
                areas_for_improvement = (
                    improvement_match.group(1).strip()
                    if improvement_match
                    else "Continue developing your skills."
                )
                recommendation = (
    ...
    def post(self, request, session_id):
        ...
        try:
            ...
            return Response(
                {
                    "session_id": str(session.id),
                    "candidate_name": session.ai_configuration.get(
                        "candidate_name", "Candidate"
                    ),
                    "job_title": session.ai_configuration.get(
    ...
    def post(self, request, session_id, question_id):
        ...
        try:
            ...
            question = get_object_or_404(
                AIInterviewQuestion, id=question_id, session=session
            )
            ...
            response = ai_interview_service.transcribe_response(
                audio_file=audio_file, session_id=session_id, question_id=question_id
            )
