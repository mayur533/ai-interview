...
class InterviewSlot(models.Model):
    ...
    def clean(self):
        if self.start_time and self.end_time:
            ...
            # Business hours validation (8:00 AM - 10:00 PM in 24-hour format)
            # Since TimeField stores time in 24-hour format, we can validate directly
            ...
            if self.start_time < business_start or self.end_time > business_end:
                raise ValidationError(
                    _("Interview time must be between 8:00 AM and 10:00 PM")
                )
