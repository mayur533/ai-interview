...
def get_camera_for_session(session_key):
    with camera_lock:
        ...
        try:
            ...
            print(
                f"Could not find session for session_key {session_key} to create camera."
            )
            return None
        except Exception as e:
...
def create_interview_invite(request):
    if request.method == "POST":
        ...
        if not all(
            [candidate_name, candidate_email, jd_text, resume_file, scheduled_at_str]
        ):
            return render(
                request,
                "interview_app/create_invite.html",
                {"error": "All fields are required.", "languages": SUPPORTED_LANGUAGES},
            )
        ...
        try:
            ...
            return render(
                ...
                "interview_app/create_invite.html",
                {
                    "error": "Invalid date and time format provided.",
        ...
        interview_url = request.build_absolute_uri(
        ...
        try:
            ...
            return render(
                ...
                "interview_app/create_invite.html",
                {
                    "error": f"Could not send email. Please check your .env settings and ensure you are using a Google App Password. Error: {e}",
                    "languages": SUPPORTED_LANGUAGES,
                },
            )
    ...
    return render(
        request, "interview_app/create_invite.html", {"languages": SUPPORTED_LANGUAGES}
    )
    ...
    if texttospeech is not None:
        try:
            ...
            voice = texttospeech.VoiceSelectionParams(
                language_code="en-IN", name="en-IN-Wavenet-F"
            )
...
def interview_portal(request):
    ...
    if session.status != "SCHEDULED":
        return render(
            request,
            "interview_app/invalid_link.html",
            {"error": "This interview has already been completed or has expired."},
        )
    if session.scheduled_at:
        ...
        print(
            f"DEBUG: Time comparison - Now: {now}, Start: {start_time}, Expiry: {expiry_time}"
        )
        print(
            f"DEBUG: Now < Start: {now < start_time}, Now > Expiry: {now > expiry_time}"
        )
        ...
        if now < (start_time - buffer_time):
            ...
            print(
                f"DEBUG: User too early, showing countdown. Start time local: {start_time_local}"
            )
            ...
            return render(
                ...
                "interview_app/invalid_link.html",
                {
                    ...
                    "scheduled_time_str": start_time_local.strftime(
                        "%Y-%m-%d %I:%M %p IST"
                    ),
        ...
        return render(
            ...
            "interview_app/invalid_link.html",
            {
                "error": "This interview session is invalid as it does not have a scheduled time."
            },
        )
    ...
    try:
        ...
        if False:  # Disable existing questions to force new AI generation
            ...
            existing_questions = session.questions.filter(
                question_level="MAIN"
            ).order_by("order")
                ...
                for i, q in enumerate(existing_questions):
                    ...
                    if not q.audio_url:
                        ...
                        tts_path = os.path.join(
                            tts_dir, f"q_{i}_{session.session_key}.mp3"
                        )
                        synthesize_speech(
                            q.question_text,
                        ...
                        audio_url = (
                            f"{settings.MEDIA_URL}tts/{os.path.basename(tts_path)}"
                        )
            ...
            coding_questions = []
            print(
                f"DEBUG: Loading existing questions. Found {session.questions.filter(question_type='CODING').count()} coding questions"
            )
            for q in session.questions.filter(question_type="CODING").order_by("order"):
                ...
                print(
                    f"DEBUG: Added coding question with ID: {coding_q['id']}, title: {coding_q['title']}"
                )

            # Check if we need to generate new questions
            print(
                f"DEBUG: generate_new_questions flag: {generate_new_questions if 'generate_new_questions' in locals() else 'NOT SET'}"
            )
            if "generate_new_questions" in locals() and generate_new_questions:
                ...
                if DEV_MODE:
                    print(
                        "--- RUNNING IN DEV MODE: Using hardcoded questions and summary. ---"
                    )
                    ...
                    all_questions = [
                        {
                            "type": "Ice-Breaker",
                            "text": "Welcome! To start, can you tell me about a challenging project you have worked on?",
                        },
                        {
                        ...
                        },
                        {
                            ...
                            "text": "Describe a time you had a conflict with a coworker and how you resolved it.",
                        },
                    ]
            ...
            if DEV_MODE:
                ...
                if not sections:
                    raise ValueError("Could not parse ## headers from AI response.")
            ...
            for i, q_data in enumerate(all_questions):
                tts_path = os.path.join(tts_dir, f"q_{i}_{session.session_key}.mp3")
                synthesize_speech(
                    q_data["text"], session.language_code, session.accent_tld, tts_path
                )
                audio_url = f"{settings.MEDIA_URL}tts/{os.path.basename(tts_path)}"
                q_data["audio_url"] = audio_url
                InterviewQuestion.objects.create(
        ...
        print(
            f"DEBUG: Context keys: {list({'session_key': session_key, 'interview_session_id': str(session.id), 'spoken_questions_data': all_questions, 'coding_questions_data': coding_questions, 'interview_started': True}.keys())}"
        )
        ...
        traceback.print_exc()
        return HttpResponse(
            f"An API or processing error occurred: {str(e)}", status=500
        )
...
def interview_report(request, session_id):
    try:
        ...
        has_spoken_answers = session.questions.filter(
            transcribed_answer__isnull=False, transcribed_answer__gt=""
        ).exists()
        has_code_submissions = session.code_submissions.exists()

        if (
            session.language_code == "en"
            and not session.is_evaluated
            and (has_spoken_answers or has_code_submissions)
        ):
            print(
                f"--- Performing all first-time AI evaluations for session {session.id} with Gemini ---"
            )
            model = genai.GenerativeModel("gemini-1.5-flash-latest")
            ...
            try:
                ...
                session.answers_feedback = (
                    "An error occurred during answers evaluation."
                )
            ...
            try:
                ...
                warning_summary = (
                    ", ".join(
                        [f"{count}x {name}" for name, count in warning_counts.items()]
                    )
                    or "None"
                )
                overall_prompt = (
                ...
                score_match = re.search(
                    r"OVERALL SCORE:\s*([\d\.]+)", overall_response_text
                )
                if score_match:
                ...
                session.overall_performance_feedback = "An error occurred."
        ...
        if session.language_code == "en":
            for item in all_questions:
                if item.transcribed_answer:
                    ...
                    read_time_minutes = read_time_result.minutes + (
                        read_time_result.seconds / 60
                    )
                    ...
                    item.filler_word_count = sum(
                        lower_answer.count(word) for word in FILLER_WORDS
                    )
                    total_filler_words += item.filler_word_count
                    sentiment_scores.append(
                        {
                            "question": f"Q{item.order + 1}",
                            "score": TextBlob(
                    ...
                    sentiment_scores.append(
                        {"question": f"Q{item.order + 1}", "score": 0.0}
                    )

        final_avg_wpm = round(avg_wpm / wpm_count) if wpm_count > 0 else 0
        final_avg_response_time = (
            round(avg_response_time / response_time_count, 2)
        ...
        analytics_data = {
            ...
            "evaluation_scores": {
                "Resume vs JD": session.resume_score or 0,
                "Interview Answers": session.answers_score or 0,
            },
            "communication_radar": {
        ...
        main_questions_with_followups = (
            session.questions.filter(
                question_level="MAIN", question_type__in=["TECHNICAL", "BEHAVIORAL"]
            )
        ...
        context = {
            ...
            "behavioral_analysis_html": mark_safe(
                (session.behavioral_analysis or "").replace("\n", "<br>")
            ),
            "keyword_analysis_html": mark_safe(
                (session.keyword_analysis or "")
                .replace("\n", "<br>")
                .replace("**", "<strong>")
                .replace("**", "</strong>")
            ),
...
def download_report_pdf(request, session_id):
    ...
    try:
        ...
        traceback.print_exc()
        return HttpResponse(
            f"An unexpected error occurred while generating the PDF report: {e}",
            status=500,
        )
...
def end_interview_session(request):
    try:
        ...
        if not session_key:
            return JsonResponse(
                {"status": "error", "message": "Session key required."}, status=400
            )
...
def submit_coding_challenge(request):
    try:
        ...
        session.save()
        print(
            f"--- Session {session_key} with coding challenge marked as COMPLETED. ---"
        )
...
def generate_and_save_follow_up(session, parent_question, transcribed_answer):
    ...
    try:
        ...
        if "NO_FOLLOW_UP" in follow_up_text or not follow_up_text:
            return None
        if len(follow_up_text) > 10:
            ...
            tts = gTTS(
                ...
                tld=session.accent_tld if session.language_code == "en" else "com",
            )
            tts_dir = os.path.join(settings.MEDIA_ROOT, "tts")
            os.makedirs(tts_dir, exist_ok=True)
            ...
            audio_url = os.path.join(
                settings.MEDIA_URL, "tts", os.path.basename(tts_path)
            )
    ...
    if request.method == "POST" and request.FILES.get("audio_data"):
        ...
        try:
            ...
            if session_id and question_id:
                try:
                    question_to_update = InterviewQuestion.objects.get(
                    ...
                    if (
                        transcribed_text
                        and question_to_update.question_level == "MAIN"
                        and question_to_update.session.language_code == "en"
                    ):
                    ...
                    print(
                        f"Warning: Could not find question with ID {question_id} to save answer."
                    )
            os.remove(full_path)
            return JsonResponse(
                {"text": transcribed_text, "follow_up_question": follow_up_data}
            )
        except Exception as e:
...
def verify_id(request):
    try:
        ...
        name_verified = (
            ...
            and not any(phrase in name.lower() for phrase in invalid_phrases)
        )
        ...
        if name == "SKIPPED":
            ...
            return JsonResponse(
                {
                    "status": "error",
                    "message": f"Could not reliably read the name from the ID card. Extracted: '{name}'. Please try again.",
                }
            )
            ...
            return JsonResponse(
                {
                    "status": "error",
                    "message": f"Name on ID ('{name}') does not match the registered name ('{session.candidate_name}').",
                }
            )
...
def run_subprocess_windows(command, cwd=None, input_data=None):
    try:
        ...
        return subprocess.CompletedProcess(
            command, 1, stdout=None, stderr="Execution timed out."
        )
    except Exception as e:
        return subprocess.CompletedProcess(
            command, 1, stdout=None, stderr=f"Server execution error: {str(e)}"
        )
...
def execute_csharp_windows(code, test_input):
    with tempfile.TemporaryDirectory() as temp_dir:
        subprocess.run(
            ["dotnet", "new", "console", "--force"], cwd=temp_dir, capture_output=True
        )
...
def execute_code(request):
    ...
    if isinstance(question_id, int):
        ...
        question = InterviewQuestion.objects.filter(
            session=session, order=question_id - 1
        ).first()
        if not question:
            return JsonResponse(
                {"status": "error", "message": "Question not found."}, status=400
            )
    else:
    ...
    if question.coding_language == "PYTHON":
        ...
        return JsonResponse(
            {"status": "error", "output": "Language not supported for testing."}
        )
    ...
    if language in lang_map:
        ...
        return JsonResponse(
            {"status": "error", "output": f"Language '{language}' is not supported."},
            status=400,
        )

    if stderr:
        return JsonResponse(
            {"status": "error", "output": stderr.strip(), "passed": False}
        )
    else:
        actual_output = stdout.strip() if stdout else ""
        ...
        return JsonResponse(
            {"status": "success", "output": actual_output, "passed": passed}
        )
...
def submit_coding_challenge(request):
    try:
        ...
        session = get_object_or_404(
            InterviewSession, session_key=data.get("session_key")
        )
        ...
        )  # Get language from the question object for security
        ...
        session.save()
        print(
            f"--- Session {session.session_key} with coding challenge marked as COMPLETED. ---"
        )
        ...
        try:
            ...
            for question in all_questions:
                question_data = {
                    ...
                    "audio_url": (
                        question.audio_url if hasattr(question, "audio_url") else None
                    ),
                }
            ...
            analytics_data = {
                ...
                "communication_metrics": {
                    ...
                    "answered_questions": len(
                        [q for q in all_questions if q.transcribed_answer]
                    ),
    ...
    def get(self, request):
        ...
        try:
            ...
            for session in sessions:
                ...
                answered_questions = (
                    all_questions.filter(transcribed_answer__isnull=False)
                    .exclude(transcribed_answer="")
                    .count()
                )
            ...
            return Response(
                {"sessions": sessions_data, "total_count": len(sessions_data)},
