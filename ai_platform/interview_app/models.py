...
class WarningLog(models.Model):
    session = models.ForeignKey(
        InterviewSession, related_name="logs", on_delete=models.CASCADE
    )
...
class InterviewQuestion(models.Model):
    session = models.ForeignKey(
        InterviewSession, related_name="questions", on_delete=models.CASCADE
    )
    ...
    question_type = models.CharField(
        max_length=50, choices=QUESTION_TYPE_CHOICES, default="TECHNICAL"
    )
...
class TestCase(models.Model):
    ...
    input_data = models.TextField(
        help_text="Input for the function, e.g., '5' for factorial(5)"
    )
    expected_output = models.TextField(help_text="Expected result, e.g., '120'")
    is_hidden = models.BooleanField(
        default=False,
        help_text="Hidden tests are not shown to the candidate but are used for final scoring.",
    )
...
class CodeSubmission(models.Model):
    session = models.ForeignKey(
        InterviewSession, related_name="code_submissions", on_delete=models.CASCADE
    )
    question_id = models.CharField(
        max_length=36, help_text="Question ID (can be UUID or integer)"
    )
    ...
    output_log = models.TextField(
        null=True,
        blank=True,
        help_text="Stores the results of running against all test cases.",
    )
    created_at = models.DateTimeField(auto_now_add=True)
