...
def interview_sessions_api(request):
    ...
    try:
        ...
        if search:
            sessions = (
                sessions.filter(candidate_name__icontains=search)
                | sessions.filter(candidate_email__icontains=search)
                | sessions.filter(session_key__icontains=search)
            )
...
def interview_questions_api(request):
    ...
    try:
        ...
        questions = (
            ...
            .all()
            .order_by("-session__created_at", "order")
        )
        ...
        if search:
            ...
            ) | questions.filter(session__candidate_name__icontains=search)
...
def code_submissions_api(request):
    ...
    try:
        ...
        # Base queryset with session data
        submissions = (
            CodeSubmission.objects.select_related("session")
            .all()
            .order_by("-created_at")
        )
        ...
        if search:
            ...
            ) | submissions.filter(session__candidate_name__icontains=search)
...
def warning_logs_api(request):
    ...
    try:
        ...
        # Base queryset with session data
        warnings = (
            WarningLog.objects.select_related("session").all().order_by("-timestamp")
        )
...
def dashboard_stats_api(request):
    ...
    try:
        ...
        behavioral_questions = InterviewQuestion.objects.filter(
        ...
        passed_submissions = CodeSubmission.objects.filter(
            passed_all_tests=True
        ).count()
        failed_submissions = CodeSubmission.objects.filter(
        ...
        avg_overall_score = (
            ...
            ).aggregate(avg_score=models.Avg("overall_performance_score"))["avg_score"]
            or 0
        )

        avg_answers_score = (
            InterviewSession.objects.filter(answers_score__isnull=False).aggregate(
                avg_score=models.Avg("answers_score")
            )["avg_score"]
            or 0
        )

        avg_resume_score = (
            InterviewSession.objects.filter(resume_score__isnull=False).aggregate(
                avg_score=models.Avg("resume_score")
            )["avg_score"]
            or 0
        )
