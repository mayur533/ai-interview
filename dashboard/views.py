...
class AnalyticsSummaryView(APIView):
    """Get analytics summary for hiring agency statistics"""
    ...
    def get(self, request):
        ...
        try:
            ...
            if user_role == "COMPANY":
                base_filters["candidate__job__company_name"] = getattr(request.user, "company_name", "")
            elif user_role in ["HIRING_AGENCY", "RECRUITER"]:
                base_filters["candidate__recruiter"] = request.user
            
            # Get all hiring agencies (users with HIRING_AGENCY or RECRUITER role)
            hiring_agencies = CustomUser.objects.filter(
                role__in=["HIRING_AGENCY", "RECRUITER"]
            ).distinct()
            
            # Calculate hiring agency statistics
            agency_stats = []
            for agency in hiring_agencies:
                candidates = Candidate.objects.filter(recruiter=agency)
                interviews = Interview.objects.filter(candidate__recruiter=agency)
                
                # Counts by status
                uploaded = candidates.count()
                selected = candidates.filter(status__in=["HIRED", "OFFERED", "INTERNAL_INTERVIEWS"]).count()
                rejected = candidates.filter(status="REJECTED").count()
                
                # Interview counts
                interviews_scheduled = interviews.count()
                interviews_selected = interviews.filter(
                    candidate__status__in=["HIRED", "OFFERED", "INTERNAL_INTERVIEWS"]
                ).count()
                interviews_rejected = interviews.filter(candidate__status="REJECTED").count()
                
                agency_stats.append({
                    "agency_name": agency.company_name or agency.username,
                    "recruiter_name": agency.full_name or agency.username,
                    "uploaded_profiles": uploaded,
                    "selected_profiles": selected,
                    "rejected_profiles": rejected,
                    "selection_rate": (selected / uploaded * 100) if uploaded > 0 else 0,
                    "interviews_scheduled": interviews_scheduled,
                    "interviews_selected": interviews_selected,
                    "interviews_rejected": interviews_rejected,
                    "interview_selection_rate": (interviews_selected / interviews_scheduled * 100) if interviews_scheduled > 0 else 0,
                })
            
            # Sort by uploaded profiles for top agencies
            agency_stats.sort(key=lambda x: x["uploaded_profiles"], reverse=True)
            ...
            summary = {
                ...
                "all_agencies": agency_stats,
                "top_5_agencies": top_5_agencies,
            }
            
            ActionLogger.log_user_action(
                user=request.user,
                action="analytics_summary_view",
                details={"agencies_count": len(agency_stats)},
                status="SUCCESS",
            )
            ...
            ActionLogger.log_user_action(
                user=request.user,
                action="analytics_summary_error",
                details={"error": str(e)},
                status="FAILED",
            )
            return Response(
    ...
    def get(self, request):
        ...
        try:
            ...
            summary = {
                ...
                "total_interviews": dashboard_data["interview_stats"][
                    "total_interviews"
                ],
                "total_candidates": dashboard_data["candidate_stats"][
                    "total_candidates"
                ],
    ...
    def get(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                return Response(
                    dashboard_data["resume_stats"], status=status.HTTP_200_OK
                )
    ...
    def get(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                return Response(
                    dashboard_data["interview_stats"], status=status.HTTP_200_OK
                )
    ...
    def get(self, request):
        ...
        if serializer.is_valid():
            ...
            try:
                ...
                return Response(
                    dashboard_data["candidate_stats"], status=status.HTTP_200_OK
                )
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            try:
                ...
                if not created:
                    widget.position = serializer.validated_data.get(
                        "position", widget.position
                    )
                    widget.is_enabled = serializer.validated_data.get(
                        "is_enabled", widget.is_enabled
                    )
                    widget.settings = serializer.validated_data.get(
    ...
    def get(self, request, chart_type):
        ...
        try:
            ...
            if chart_type == "resume_trend":
                ...
                return Response(
                    {"error": "Invalid chart type"}, status=status.HTTP_400_BAD_REQUEST
                )
        ...
        if serializer.is_valid():
            try:
                ...
                export_data = {
                    ...
                    "summary": {
                        "total_resumes": dashboard_data["resume_stats"][
                            "total_uploads"
                        ],
                        "total_interviews": dashboard_data["interview_stats"][
                            "total_interviews"
                ...
                if include_charts:
                    ...
                    export_data["charts"] = {
                        ...
                        "interview_trend": self._get_interview_trend_data(
                            request.user, days
                        ),
                        "candidate_distribution": self._get_candidate_distribution_data(
                            request.user, days
                        ),
                    }
